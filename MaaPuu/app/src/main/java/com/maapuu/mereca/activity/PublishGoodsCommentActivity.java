...
public class PublishGoodsCommentActivity extends BaseActivity {
    ...
    private void initRv() {
        ...
        adapter.setOnItemClickListener(new GridImageAdapter.OnItemClickListener() {
            ...
            public void onItemClick(int position, View v) {
                PictureSelector.create(PublishGoodsCommentActivity.this).externalPicturePreview(position, adapter.getList());
            }
        });
    }
        ...
        switch (view.getId()){
            ...
            case R.id.txt_commit:
                if(ratingbar1.getRating() == 0){
                    ToastUtil.show(mContext,"请为描述相符评分");
                    return;
                }
                if(ratingbar2.getRating() == 0){
                    ToastUtil.show(mContext,"请为物流服务评分");
                    return;
                }
    ...
    private void publish(boolean bool) {
        ...
        buffer.append("]");
        mHttpModeBase.xPost(HttpModeBase.HTTP_REQUESTCODE_1, UrlUtils.order_commodity_evl_set(LoginUtil.getInfo("token"),LoginUtil.getInfo("uid"),oid,
                adapter.getList().size() == 0 ? "" : buffer.toString(), EmojiUtil.stringToUtf8(etContent.getText().toString()).replace("+","%20"),ratingbar1.getRating()+"",
                ratingbar2.getRating()+"",ratingbar3.getRating()+"",txtHidden.isSelected()?1:0),bool);
    }
    ...
    private GridImageAdapter.onAddPicClickListener onAddPicClickListener = new GridImageAdapter.onAddPicClickListener() {
        ...
        public void onAddPicClick() {
            PictureSelector.create(PublishGoodsCommentActivity.this)
