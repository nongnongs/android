...
public class ShopDataFragment extends BaseFragment {
    ...
    protected int setContentViewById() {
        return R.layout.shop_fragment_work_data;
    }
    ...
    public void refresh() {
        mHttpModeBase.xPost(HttpModeBase.HTTP_REQUESTCODE_1, UrlUtils.shop_report_data_get(LoginUtil.getInfo("token"),LoginUtil.getInfo("uid"),shop_id),true);
    }
...
//        txtDayTurnover.setText(month.getAvg_day_wage()+"元");
    ...
    private void setPieChart(IncomeChildBean bean) {
        ...
        listView.setAdapter(new QuickAdapter<IncomePointBean>(mContext,R.layout.layout_income_analysis_list_item,bean.getCatalog()) {
            @Override
            protected void convert(BaseAdapterHelper helper, IncomePointBean item) {
