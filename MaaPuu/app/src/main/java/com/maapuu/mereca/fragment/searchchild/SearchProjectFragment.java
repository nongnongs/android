...
public class SearchProjectFragment extends BaseFragment {
                ...
                try {
                    ...
                    if (object.optInt("status") == 1) {
                        ...
                        if(!StringUtils.isEmpty(resultObj.optString("rlt_data")) && resultObj.optJSONArray("rlt_data").length() > 0){
                            ...
                            List<ProjectBean> lsJson = FastJsonTools.getPersons(resultObj.optString("rlt_data"),ProjectBean.class);
                            setAdapter(lsJson);
                            page ++ ;
                        }else {
    ...
    private void setAdapter(List<ProjectBean> lsJson) {
        ...
        list.addAll(lsJson);
        if(adapter == null){
            adapter = new BaseRecyclerAdapter<ProjectBean>(mContext,list,R.layout.layout_search_project_item) {
                @Override
