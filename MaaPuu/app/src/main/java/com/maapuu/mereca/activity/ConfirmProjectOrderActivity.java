...
public class ConfirmProjectOrderActivity extends BaseActivity {
    ...
    public static ConfirmProjectOrderActivity activity;
    ...
    public void initView() {
        ...
        item_shop_id = getIntent().getStringExtra("item_shop_id");
        list = new ArrayList<>();
    }

    @Override
    public void initData() {
        mHttpModeBase.xPost(HttpModeBase.HTTP_REQUESTCODE_1, UrlUtils.project_order_init_get(LoginUtil.getInfo("token"),LoginUtil.getInfo("uid"),
                AppConfig.LAT,AppConfig.LNG,item_shop_id),true);
    }
            ...
            case HttpModeBase.HTTP_REQUESTCODE_1:
                try {
                    ...
                    if (object.optInt("status") == 1) {
                        ...
                        if(resultObj.has("item_data") && !StringUtils.isEmpty(resultObj.optString("item_data"))){
                            InitProjectOrderBean bean = FastJsonTools.getPerson(resultObj.optString("item_data"),InitProjectOrderBean.class);
                            list.clear();
                            list.add(bean);
                            if(list.size()>0){
                                txtGoodsNum.setText("共"+list.get(0).getSrv_num()+"项服务");
                            }
                            setAdapter();
                        }
                        ...
                        if(resultObj.has("order_calc_rlt") && !StringUtils.isEmpty(resultObj.optString("order_calc_rlt"))){
                            ...
                            if(red_amount+member_discount > order_amount){
                                txtYouhuiAmount.setText(StringUtils.formatDouble(order_amount));
                            }else {
                                txtYouhuiAmount.setText(StringUtils.formatDouble(member_discount+red_amount));
                            }
...
//                helper.setText(R.id.txt_goods_num,"×"+item.getSrv_num());
    ...
    public void onClick(View view) {
        switch (view.getId()){
            ...
            case R.id.ll_coupon:
                ...
                it.putExtra("item_shop_ids",item_shop_id);
            ...
            case R.id.txt_commit:
                mHttpModeBase.xPost(HttpModeBase.HTTP_REQUESTCODE_2, UrlUtils.project_order_set(LoginUtil.getInfo("token"),LoginUtil.getInfo("uid"),
                        item_shop_id,red_id,member_id,StringUtils.formatDouble(pay_amount),round_num),true);
                break;
        }
    }
    ...
    public void doAlipay(String partner,String seller_id,String subject,String body,String out_trade_no,String notify_url,String amount) {
        ...
        Runnable payRunnable = new Runnable() {
            ...
            public void run() {
                // 构造PayTask 对象
                SimpleHUD.dismiss();
                PayTask alipay = new PayTask(ConfirmProjectOrderActivity.this);
                // 调用支付接口，获取支付结果
    ...
    private void calculate(){
        mHttpModeBase.xPost(HttpModeBase.HTTP_REQUESTCODE_4,UrlUtils.project_order_calc_get(LoginUtil.getInfo("token"),LoginUtil.getInfo("uid"),
                StringUtils.formatDouble(order_amount),StringUtils.formatDouble(red_amount),cardBean == null?"0":(cardBean.getCard_type()+""),
                cardBean == null?"0":cardBean.getDiscount(), cardBean == null?"0":cardBean.getRemain_amount()),true);
    }
