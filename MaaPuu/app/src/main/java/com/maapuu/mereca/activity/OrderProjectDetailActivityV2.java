...
public class OrderProjectDetailActivityV2 extends BaseActivity {
    ...
    private QuickAdapter<OrderChildBean> adapter;
    ...
    public static OrderProjectDetailActivityV2 activity;
    ...
    public void initContentView(Bundle savedInstanceState) {
        setContentView(R.layout.activity_project_order_detail_v2);
        activity = this;
    }
    ...
    public void initData() {
        mHttpModeBase.xPost(HttpModeBase.HTTP_REQUESTCODE_1, UrlUtils.project_order_detail_new_get(LoginUtil.getInfo("token"),LoginUtil.getInfo("uid"),
                ...
                try {
                    ...
                    if (object.optInt("status") == 1) {
                        ...
                        if(resultObj.has("items") && !StringUtils.isEmpty(resultObj.optString("items")) && !(resultObj.opt("items") instanceof Boolean)){
                            ...
                            order_detail_id = list.get(0).getOrder_detail_id();
                            if("1".equals(list.get(0).getIs_refund())){
                                ...
                                txtRefundStatus.setText(list.get(0).getRefund_text());
                            }
                        }
                        setAdapter();
    ...
    private void setAdapter() {
        adapter = new QuickAdapter<OrderChildBean>(mContext,R.layout.layout_project_order_detail_v2_item,list) {
            @Override
            protected void convert(BaseAdapterHelper helper, OrderChildBean item) {
                UIUtils.loadImg(mContext,item.getItem_img(), (ImageView) helper.getView(R.id.iv_goods));
                helper.setText(R.id.txt_goods_name,item.getItem_name());
                helper.setText(R.id.txt_goods_spec,item.getItem_desc());
                helper.setText(R.id.txt_goods_price,"¥"+item.getPrice());
    ...
    public void onClick(View view) {
        switch (view.getId()){
            ...
            case R.id.txt_apply_refund:
                if(!StringUtils.isEmpty(order_detail_id)){
    ...
    public void doAlipay(String partner,String seller_id,String subject,String body,String out_trade_no,String notify_url,String amount) {
        ...
        Runnable payRunnable = new Runnable() {
            ...
            public void run() {
                // 构造PayTask 对象
                SimpleHUD.dismiss();
                PayTask alipay = new PayTask(OrderProjectDetailActivityV2.this);
                // 调用支付接口，获取支付结果
