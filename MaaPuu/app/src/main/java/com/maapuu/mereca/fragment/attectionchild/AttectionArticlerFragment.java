...
import com.maapuu.mereca.adapter.AttectionArticlerAdapter;
...
public class AttectionArticlerFragment extends BaseFragment {
    ...
    private AttectionArticlerAdapter adapter;
    ...
    private void initData(int pageNum) {
        mHttpModeBase.xPost(HttpModeBase.HTTP_REQUESTCODE_1, UrlUtils.attention_list_get(LoginUtil.getInfo("token"),LoginUtil.getInfo("uid"),"3",pageNum),true);
    }
                ...
                try {
                    ...
                    if (object.optInt("status") == 1) {
                        if(!StringUtils.isEmpty(object.optString("result")) && object.optJSONArray("result").length() > 0){
                            ...
                            List<ArticlerBean> lsJson = FastJsonTools.getPersons(object.optString("result"),ArticlerBean.class);
                            setAdapter(lsJson);
                            page ++ ;
                        }else {
                            List<ArticlerBean> lsJson = new ArrayList<>();
                            setAdapter(lsJson);
                            if(page != 1){
    ...
    private void setAdapter(List<ArticlerBean> lsJson) {
        if(page == 1){
            list.clear();
        }
        list.addAll(lsJson);
        if(adapter == null){
            adapter = new AttectionArticlerAdapter(mContext,list);
        ...
        adapter.setOnItemClickListener(new AttectionArticlerAdapter.OnRecyclerViewItemClickListener() {
            @Override
            ...
            public void onRightItemClick(View v, int position) {
                ...
                mHttpModeBase.xPost(HttpModeBase.HTTP_REQUESTCODE_2,UrlUtils.attention_set(LoginUtil.getInfo("token"),LoginUtil.getInfo("uid"),"3",
                        list.get(position).getUid(), "0"),true);
            }
        });
    }
